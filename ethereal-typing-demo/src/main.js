...
import { clamp, smoothstep, lerp, makeSoftParticleDataURL, makeGrainDataURL } from './utils.js';
...
let targetPixelRatio = Math.min(window.devicePixelRatio || 1, CONFIG.renderer.maxPixelRatio);
...
const padDensity = new XYPad(densityContainer, 'pad-density', 'Density / Energy', { x: 0.83, y: 0.64 });
const padBlur = new XYPad(blurContainer, 'pad-blur', 'Blur / Glow', { x: 0.77, y: 0.41 });
const padFog = new XYPad(fogContainer, 'pad-fog', 'Fog Scale / Pulse', { x: 0.94, y: 1.00 });
let xy = { x: padDensity.getX(), y: padDensity.getY() };
padDensity.onChange((x, y) => { xy.x = x; xy.y = y; });

// FPS meter (rolling avg)
const fpsDiv = document.getElementById('fps');
let lastFpsUpdate = 0;
let accum = 0; let frames = 0;
...
window.addEventListener('keydown', (e) => {
  ...
  const glowBoost = (window.__impactGain != null ? window.__impactGain : 1.0) * autoImpact * lerp(0.5, 3.0, padBlur.getY());
...
function loop(t) {
  ...
  let autoImpact = 1.0, autoPush = 1.0;
  if (autoOn) {
  ...
  const blurPx = lerp(blurMin, blurMax, Math.min(1, intensity * autoImpact)).toFixed(2) + 'px';
  ...
  if (t - lastFpsUpdate > 300) {
    ...
    lastFpsUpdate = t; accum = 0; frames = 0;
  }
...
function updateFogUI() {
  ...
  glass.style.background = `rgba(255,255,255,${gA.toFixed(3)})`;
  // Blur bounds labels
  ...
  // Auto mode readout
  if (autoSensEl && autoSensVal) autoSensVal.textContent = (parseFloat(autoSensEl.value) || 0).toFixed(2);
  if (shockRadiusEl && shockRadiusVal) shockRadiusVal.textContent = (parseFloat(shockRadiusEl.value)||0.42).toFixed(2);
  if (vignetteEl && vignetteVal) vignetteVal.textContent = (parseFloat(vignetteEl.value)||0.12).toFixed(2);
  if (fogSystem && shockRadiusEl) {
    for (const l of fogSystem.layers) l.mesh.material.uniforms.uShockRadius.value = parseFloat(shockRadiusEl.value)||0.42;
  }
  ...
  if (burstAlphaEl && burstAlphaVal) {
    window.__burstAlpha = Math.max(0, Math.min(1, parseFloat(burstAlphaEl.value) || 0.25));
...
const allControlElements = [
  fogAlpha, fogScale, fogPulse, fogSpeed, fogPush, fogRot, fogExpand, bgDark,
  blurMinEl, blurMaxEl, glassAlpha, grainOpacity, grainScale, grainSpeed,
  partBright, partCount, partSize, minSat, hueJitter, lightStrength, impact,
  meterDecayEl, burstAlphaEl, meterIncEl, shockRadiusEl, vignetteEl
];
...
const CONTROL_KEYS = [
  'fogAlpha','fogScale','fogPulse','fogSpeed','fogPush','fogRot','fogExpand','bgDark','blurMin','blurMax','glassAlpha','grainOpacity','grainScale','grainSpeed','lightStrength','partBright','impact','partCount','partSize','minSat','hueJitter','autoMode','autoSens','meterDecay','burstAlpha','meterInc','shockRadius','vignette'
];
...
resetBtn.addEventListener('click', () => {
  const defaults = {
    fogAlpha: '0.00', fogScale: '1.20', fogPulse: '0.00', fogSpeed: '2.00', fogPush: '0.00', fogRot: '2.00', fogExpand: '1.00',
    bgDark: '0.00', blurMin: '24', blurMax: '10', glassAlpha: '0.06', grainOpacity: '0.00', grainScale: '256', grainSpeed: '18',
    lightStrength: '2.00', partBright: '0.40', impact: '0.75', partCount: '0.50', partSize: '1.20', minSat: '0.75', hueJitter: '8', 
    meterDecay: '0.05', burstAlpha: '0.15', meterInc: '0.08', shockRadius: '0.42', vignette: '0.12', autoMode: 'true', autoSens: '0.60'
  };
  for (const k of CONTROL_KEYS) {
...
function applyPreset(name){
  ...
  // Apply slider values
  const apply = (k,v)=>{ const el=document.getElementById(k); if (el){ el.value=v; }};
  ...
  if (P.padSettings) {
    if (P.padSettings.density) {
      ...
      padDensity._render();
      padDensity._emit();
      localStorage.setItem('ethereal_xy_pad-density', JSON.stringify({ x: padDensity.x, y: padDensity.y }));
    }
    if (P.padSettings.blur) {
      ...
      padBlur._render();
      padBlur._emit();
      localStorage.setItem('ethereal_xy_pad-blur', JSON.stringify({ x: padBlur.x, y: padBlur.y }));
    }
    if (P.padSettings.fog) {
      ...
      padFog._render();
      padFog._emit();
      localStorage.setItem('ethereal_xy_pad-fog', JSON.stringify({ x: padFog.x, y: padFog.y }));
    }
  }
  ...
  updateFogUI();
  // visual active state
  for (const btn of presetBar.querySelectorAll('button')) btn.classList.remove('preset-active');
...
window.addEventListener('keydown', (e)=>{ if (e.key === '=') toggleUI(); });
