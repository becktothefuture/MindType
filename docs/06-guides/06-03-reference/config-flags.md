...
- ACTIVE_REGION_WORDS_DEFAULT: 20 (behind caret; single region).
...
- ACTIVE_REGION_WORDS: default 20 (configurable; see `config/defaultThresholds.ts`)
...
- UNDO_EXTERNAL: true (host/editor manages undo). A dedicated rollback hotkey reverts the last wave.
