...
Feature: Streamed diffusion (REQ-STREAMED-DIFFUSION, REQ-ACTIVE-REGION)
  Scenario: Frontier advances during typing
    ...
    And applied fixes occur word-by-word inside the active region

  Scenario: Catch up on pause
    Given the user stops typing
    When SHORT_PAUSE_MS elapses
    Then diffusion catches up until the active region reaches the caret
    And the active region then contracts until typing resumes
  ...
  Scenario: Reduced motion compliance
    ...
    And a gentle static active region or fade is used instead
