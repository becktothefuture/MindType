...
async function simulateTokenGeneration(tier: DeviceTier, tokenCount: number): Promise<void> {
...
function generateTestData(size: number): string[] {
  ...
  for (let i = 0; i < size; i++) {
    data.push(`Test data item ${i} with some additional content to increase memory usage`);
  }
  return data;
}
