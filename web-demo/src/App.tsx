...
function App() {
  ...
  // ⟢ Disable auto-typing by default - let users type naturally
  ...
  useEffect(() => {
    ...
    // Boot pipeline (but don't start until user actually types)
    const pipeline = boot();
    // ⟢ Don't start pipeline immediately - wait for user input
    // pipeline.start();  // Only start when user types
  ...
  // ⟢ Initialize caret position on app load (RESPECT USER'S NATIVE CARET - don't force it)
  useEffect(() => {
    const ta = textareaRef.current;
    if (ta) {
      // Only track the current caret position, don't force it
      caretRef.current = ta.selectionStart ?? ta.value.length;
      // ⟢ Do NOT call setSelectionRange on load - let user place caret naturally
      console.log('[App] Tracking initial caret position:', caretRef.current);
    }
  }, []);
  ...
  const handleTextChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {                                                                         
    ...
    // ⟢ Start pipeline on first user keystroke (not on page load)
  ...
  const handlePresetChange = useCallback((preset: DemoPreset) => {
    ...
    // Update textarea (but preserve caret position if user has placed it)
    const ta = textareaRef.current;
    if (ta) {
      const currentCaret = ta.selectionStart ?? 0;
      ta.value = preset.text;
      // ⟢ Only move caret if user hasn't explicitly positioned it
      // If text was just replaced, move to end; otherwise preserve user's position
      if (currentCaret >= (text.length - 1)) {
        // User was near end, move to new end
        ta.setSelectionRange(newCaret, newCaret);
      } else {
        // User had caret in middle, try to preserve relative position
        const relativePos = Math.min(currentCaret / Math.max(1, text.length), 1);
        const preservedCaret = Math.round(relativePos * newCaret);
        ta.setSelectionRange(preservedCaret, preservedCaret);
        caretRef.current = preservedCaret;
      }
      ta.focus();
    }
  ...
  const runCorrections = useCallback(() => {
    ...
    // ⟢ Don't force caret to end - respect user's current position
    // Only move caret if user explicitly wants to run corrections at end
    const currentCaret = ta.selectionStart ?? text.length;
    caretRef.current = currentCaret;
    // ⟢ Removed forced caret movement - let user place caret freely
    ta.focus();
    ...
    // Force a sweep by ingesting current text with current caret position
    pipeline.ingest(text, currentCaret);
  ...
  const applyFromEvent = useCallback((detail: { start: number; end: number; text: string }) => {                                                                
    ...
    try {
      // ⟢ Get current native caret position before applying correction
      const currentNativeCaret = ta.selectionStart ?? 0;
      const newText = replaceRange(ta.value, detail.start, detail.end, detail.text, currentNativeCaret);                                                          
      setText(newText);
      ta.value = newText;
      
      // ⟢ Restore caret position after correction (caret-safe operation)
      // The replaceRange function calculates the new caret position
      // Only set if it's different to avoid interfering with user's caret placement
      if (caretRef.current !== currentNativeCaret) {
        ta.setSelectionRange(caretRef.current, caretRef.current);
      } else {
        // Let browser handle caret naturally if position unchanged
        // Don't force it unless we actually moved the caret
      }
